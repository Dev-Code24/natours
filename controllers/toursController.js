...
const multerFilter = (req, file, cb) => {
  if (file.mimetype.startsWith('image')) {
    ...
    cb(
      new AppError(
        'This file is not an image. Please upload a valid image.',
        400
      ),
      false
    );
...
exports.getToursWithin = catchAsync(async (req, res, next) => {
  ...
  if (unit === 'm') radius = distance / 6378137;
  if (!lat || !lng)
    next(
      new AppError(
        'Please provide latitude and longitude in the format of latlang',
        400
      )
    );
...
exports.getDistances = catchAsync(async (req, res, next) => {
  ...
  const allowedUnits = ['m', 'km', 'mi'];
  if (!allowedUnits.includes(unit))
    next(
      new AppError('Units should be m, km and mi only. Choose from these.', 400)
    );
