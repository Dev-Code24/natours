...
const tourSchema = new mongoose.Schema(
  {
    ...
    duration: {
      type: Number,
      required: [true, `A tour must have a duration period`],
    },
    maxGroupSize: {
      type: Number,
    ...
    priceDiscount: {
      ...
      validate: {
        ...
        message: 'The discount price should less than the actucal price',
      },
    },
    summary: {
      type: String,
...
tourSchema.pre(/^find/, function (next) {
  this.populate({
    path: 'guides',
    select:
      '-__v -passwordResetExpires -passwordResetToken -role -passwordChangedAt',
  });
  next();
});
